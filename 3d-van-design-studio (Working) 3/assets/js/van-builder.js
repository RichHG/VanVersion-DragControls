...
(function($) {
    ...
    class VanBuilder {
        constructor() {
            ...
            this.attachedObject = null; // New state variable for "attached to cursor"
    ...
    // Set up event listeners after UI controls are initialized
    this.setupEventListeners();
    
    // Load models and continue initialization
    ...
    vanBuilderData.availableModels.materials.forEach(material => {
        ...
        setupEventListeners() {
    // Attach events for handling cursor and placement
    this.sceneManager.renderer.domElement.addEventListener('mousemove', (event) => this.handleMouseMove(event));
    this.sceneManager.renderer.domElement.addEventListener('click', (event) => this.handleMouseClick(event));
    
    // Van model selection
    $('#van-models-container').on('click', '.van-builder-model-item', (event) => {
        const modelId = $(event.currentTarget).data('model-id');
        this.loadVanModel(modelId);
    });
    
    // Furniture model selection
    $('#furniture-models-container').on('click', '.van-builder-model-item', (event) => {
        const modelId = $(event.currentTarget).data('model-id');
        this.addFurnitureItem(modelId);
    });
    
    // Material selection
    $('#materials-container').on('click', '.van-builder-material-item', (event) => {
        const materialId = $(event.currentTarget).data('material-id');
        if (this.selectedObject) {
            this.applyMaterial(this.selectedObject, materialId);
        }
    });
    ...
    if (self.selectedObject) {
        ...
        if (modelData) {
            this.modelLoader.loadModel(modelData.file, (model) => {
                ...
                model.userData.modelId = modelId;
                // Attach to cursor
                this.attachedObject = model;
                this.sceneManager.addToScene(this.attachedObject);
            });
        }
    }
